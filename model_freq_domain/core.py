...
def compressor_time_averaged(audio,sr,threshold, ratio,makeup, attack_time,  release_time, downsample_factor = 16.0, if_save_fig = False):
	...
	print("===========audio_db shape:", audio_db.shape)
	print("gain_downsampled_smoothed_upsampled shape:", gain_downsampled_smoothed_upsampled[:, :gain.shape[1], 0].shape)
	print("gain shape:", gain.shape)

	# if audio_db.shape[1] != gain_downsampled_smoothed_upsampled.shape[1]:
		# audio_db = audio_db[:, :gain_downsampled_smoothed_upsampled.shape[1]]
		# audio = audio[:, :gain_downsampled_smoothed_upsampled.shape[1]]
		
	# min_len = min(audio_db.shape[1], gain_downsampled_smoothed_upsampled.shape[1])
	# gain = gain[:, :min_len]
	# gain_downsampled_smoothed_upsampled = gain_downsampled_smoothed_upsampled[:, :min_len, :]
	# audio_db = audio_db[:, :min_len]
	# audio = audio[:, :min_len]

	gain = gain[:, :audio_db.shape[1]]
	gain_downsampled_smoothed_upsampled = gain_downsampled_smoothed_upsampled[:, :audio_db.shape[1], :]
